...
const headlessStr = generator.generateFile({ mode: 'headless' })
const desktopStr = generator.generateFile({ mode: 'desktop' })

const rulesJSON = JSON.stringify(generator.rules, undefined, '\t')

await fs.writeFile(
	new URL('../packages/node/udev/50-logi-mx-creative-console-user.rules', import.meta.url),
	desktopStr,
	'utf8',
)
await fs.writeFile(
	new URL('../packages/node/udev/50-logi-mx-creative-console-headless.rules', import.meta.url),
	headlessStr,
	'utf8',
)

await fs.writeFile(
	new URL('../packages/webhid/udev/50-logi-mx-creative-console-user.rules', import.meta.url),
	desktopStr,
	'utf8',
)
