...
import { requestMXCreateConsoleDevices, reopenMXCreativeConsoleDevices } from '@logitech-mx-creative-console/webhid'
...
if (consentButton) {
	const doLoad = async () => {
		...
		const devices = await reopenMXCreativeConsoleDevices()
		if (devices.length > 0) {
			device = devices[0]
